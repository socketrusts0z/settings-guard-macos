...
The app is intentionally a best-effort commitment device, not a security boundary. A root LaunchDaemon checks for the System Settings process four times per second. Unless a temporary authorization is active, the daemon immediately terminates it. The Settings window may appear briefly before closing.
...
- A correct password permits System Settings for five minutes.
- After five minutes, the daemon closes System Settings and resumes guarding it.
- Failed password attempts use an exponential cooldown capped at five minutes.
- The state survives app closure, logout, and reboot.
